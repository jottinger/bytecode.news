...
export async function generateMetadata({
  ...
  return buildPublicMetadata({
    title: display,
    description: `Articles tagged "${display}" on bytecode.news`,
    path: `/tags/${encodeURIComponent(display)}`,
  });
}
...
export default async function TagPage({
  ...
  try {
    list = await listPostsByTag(tagName, safePage, 20);
  } catch {
    return (
        ...
        <header className="mb-10">
          ...
          <div className="pt-6 pb-8 border-b border-border/40">
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="section-label text-muted-foreground">/</span>
          ...
          <div className="mt-10 flex justify-between items-center border-t border-border/40 pt-6">
            <div>
              {safePage > 0 && (
                <Link
                  className="pagelink"
                  href={`/tags/${encodeURIComponent(tagName)}?page=${safePage - 1}`}
                >
                  &larr; Previous
                </Link>
              )}
