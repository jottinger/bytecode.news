  ...
  container.querySelectorAll(".reply-toggle").forEach((a) => {
    a.addEventListener("click", async (e) => {
      ...
      if (target.hidden) {
        ...
        attachFormHandler(target, postSlug, editor);
      } else {
        target.hidden = true;
        target.innerHTML = "";
      }
  ...
  container.querySelectorAll(".edit-toggle").forEach((a) => {
    a.addEventListener("click", async (e) => {
      ...
      bodyEl.innerHTML = `<form class="comment-edit-form" data-comment-id="${commentId}">
        <textarea name="markdownSource" rows="3"></textarea>
        <button type="submit">Save</button>
        <button type="button" class="outline cancel-edit">Cancel</button>
      </form>`;
      const { createEditor } = await import("../editor.js");
      const textarea = bodyEl.querySelector("textarea");
      const editor = createEditor(textarea, { compact: true, initialValue: existingSource });
      ...
      form.addEventListener("submit", async (ev) => {
        ...
        try {
          await put(`/comments/${commentId}`, { markdownSource: editor.value() });
          window.dispatchEvent(new PopStateEvent("popstate"));
        } catch (err) {
...
function attachFormHandler(container, postSlug, editor) {
